...
//			_plotBounds = null;
        ...
        public Site SiteForPoint(Vector2 point)
        {
            foreach (var site in _sites.Sites)
            {
                var region = site.Region(_plotBounds);
                bool isInside = false;
                for (int i = 0, j = region.Count - 1; i < region.Count; j = i++)
                {
                    if (((region[i].y > point.y) != (region[j].y > point.y)) &&
                    (point.x < (region[j].x - region[i].x) * (point.y - region[i].y) / (region[j].y - region[i].y) + region[i].x))
                    {
                        isInside = !isInside;
                    }
                }
